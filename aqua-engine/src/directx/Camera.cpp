...
)
{
    ...
    m_focusDistance = DirectX::XMVector3Length(
        DirectX::XMVectorSubtract(XMLoadFloat3(&focus), XMLoadFloat3(&eye))
    ).m128_f32[0];
}
...
)
{
    ...
    m_matrixBuffer.Create(
        BUFFER_DEFAULT(AlignmentSize(sizeof(CameraMatrix), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT))
    );
