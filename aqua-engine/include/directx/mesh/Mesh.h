...
public:
    ...
    {
        command.List()->IASetPrimitiveTopology(
    ...
    [[nodiscard]] static std::vector<D3D12_INPUT_ELEMENT_DESC>
    ...
    bool IsHit(const Mesh &mesh)
    {
        return DirectX::XMVectorGetX(
                   DirectX::XMVector3Length(
                       DirectX::XMVectorSubtract(GetPos(), mesh.GetPos())
                   )
               )
    ...
    {
        m_transformMatrix = DirectX::XMMatrixIdentity() * DirectX::XMMatrixRotationX(angle) * m_transformMatrix;
        m_matrixBuffer.SetMatrixBuffer(m_transformMatrix * m_coordinateMatrix);
    }

    void RotY(float angle)
    {
        m_transformMatrix = DirectX::XMMatrixIdentity() * DirectX::XMMatrixRotationY(angle) * m_transformMatrix;
        m_matrixBuffer.SetMatrixBuffer(m_transformMatrix * m_coordinateMatrix);
    }

    void RotZ(float angle)
    {
        m_transformMatrix = DirectX::XMMatrixIdentity() * DirectX::XMMatrixRotationZ(angle) * m_transformMatrix;
        m_matrixBuffer.SetMatrixBuffer(m_transformMatrix * m_coordinateMatrix);
    }
