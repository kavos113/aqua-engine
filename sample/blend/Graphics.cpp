...
void Graphics::SetUp()
{
    ...
    auto &tex_manager = AquaEngine::GlobalDescriptorHeapManager::CreateShaderManager(
        "rectangle",
        10,
        D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV
    );
    ...
    rectangle->CreateShaderResourceView(std::move(tex_range), tex_manager);
    inputElement = rectangle->GetInputElementDescs();

    rootSignature2.AddStaticSampler(AquaEngine::RootSignature::DefaultStaticSampler());
...
void Graphics::Render()
{
    ...
    pipelineState2.SetToCommand(*command);
    rootSignature2.SetToCommand(*command);
