...
public:
  enum class Status { MUST_BE_REFRESHED, REFRESHED };

  enum class AnimationMode { LOOP, ONCE, INVERSE, INVERSE_LOOP };

  explicit FBXModel(std::string model_path)
      : m_status(Status::MUST_BE_REFRESHED),
        m_animationMode(AnimationMode::ONCE), m_path(std::move(model_path)) {}

  explicit FBXModel(std::string model_path, const std::string &texturePath,
                    Command &command)
      : m_texture(
            Buffer(TextureManager::LoadTextureFromFile(texturePath, command))),
        m_status(Status::MUST_BE_REFRESHED),
        m_animationMode(AnimationMode::ONCE), m_path(std::move(model_path)) {}
  ...
  void SetTexture(std::unique_ptr<D3D12_DESCRIPTOR_RANGE> texture_range,
                  DescriptorHeapSegmentManager &manager);
  void SetTexture(const std::shared_ptr<DescriptorHeapSegment> &segment,
                  int offset = 0);
  ...
  void CreateMaterialBufferView(
      ...
      DescriptorHeapSegmentManager &manager);
  void CreateMaterialBufferView(
      const std::shared_ptr<DescriptorHeapSegment> &segment, int offset = 0);

  HRESULT PlayAnimation(const std::string &name,
  ...
  void SetCurrentTime(int time) {
    m_currentTime.SetTime(0, 0, 0, time, 0,
                          m_scene->GetGlobalSettings().GetTimeMode());
  }
...
private:
  ...
  void UpdateNode(FbxNode *node, FbxTime &time, FbxAnimLayer *animLayer,
                  FbxPose *pose);
  void ReadVertexCache(const FbxMesh *mesh, const FbxTime &time);
  void ComputeShapeDeformation(FbxMesh *mesh, const FbxTime &time,
                               FbxAnimLayer *animLayer);
  void ComputeSkinDeformation(const FbxMesh *mesh, FbxTime &time,
                              FbxAMatrix &global_position, FbxPose *pose);
  void ComputeLinearDeformation(const FbxMesh *mesh, const FbxTime &time,
                                const FbxAMatrix &global_position,
                                FbxPose *pose,
                                std::vector<FbxVector4> *dst = nullptr);
  void ComputeDualQuaternionDeformation(const FbxMesh *mesh,
                                        const FbxTime &time,
                                        const FbxAMatrix &global_position,
                                        FbxPose *pose,
                                        std::vector<FbxVector4> *dst = nullptr);

  static void ComputeClusterDeformation(const FbxMesh *mesh,
                                        const FbxTime &time,
                                        const FbxAMatrix &global_position,
                                        FbxPose *pose, FbxCluster *cluster,
