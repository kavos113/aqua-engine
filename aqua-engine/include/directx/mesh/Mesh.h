...
template <class DerivedClass> class Mesh {
public:
  Mesh() = default;
  ...
  void CreateMatrixBuffer(const std::shared_ptr<DescriptorHeapSegment> &segment,
                          const int offset = 0) {
  ...
  void RotX(float angle) {
    m_transformMatrix = DirectX::XMMatrixIdentity() *
                        DirectX::XMMatrixRotationX(angle) * m_transformMatrix;
    m_matrixBuffer.SetMatrixBuffer(m_transformMatrix * m_coordinateMatrix);
  }

  void RotY(float angle) {
    m_transformMatrix = DirectX::XMMatrixIdentity() *
                        DirectX::XMMatrixRotationY(angle) * m_transformMatrix;
    m_matrixBuffer.SetMatrixBuffer(m_transformMatrix * m_coordinateMatrix);
  }

  void RotZ(float angle) {
    m_transformMatrix = DirectX::XMMatrixIdentity() *
                        DirectX::XMMatrixRotationZ(angle) * m_transformMatrix;
    m_matrixBuffer.SetMatrixBuffer(m_transformMatrix * m_coordinateMatrix);
  }
